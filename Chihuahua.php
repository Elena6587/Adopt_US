<!DOCTYPE html>
<html lang="fr-FR">
<head>
  <meta charset="UTF-8">
  <title>Adopt US</title>
  <link rel="stylesheet" href="css/Chihua.css">
</head>
<body>
  <h1>Chien</h1>
    <?php require_once 'header.php';?>

    <div class="card">
    <div class="card-image">
        <img src="Chihuahua blanc.jpg" alt="Chihuahua blanc">
    </div>
    <div class="card-content">
        <h3>Race: Chihuahua</h3>
        <h4>NOM: Blue</h4>
        <p>AGE: 10 ans</p>
        <p>SEXE: MALE</p>
        <p>TAille: 20 cm</p>
        <p>POIDS: 2 kg</p>
        <button href="Adopter.php" onclick="demanderAdoption('Blue')" class="button-slide">Adopter</a>
    </div>
</div>
<script>
function demanderAdoption(nomChien) {
    let confirmation = confirm("Confirmer l'adoption de " + nomChien + " ?");
    if (confirmation) {
        // On définit les infos du rendez-vous
        const date = "20 Mai 2026";
        const heure = "17h09";
        const lieu = "123 Rue de la Paix, Paris";
        // On envoie tout vers la page Adopter.php dans l'URL
        window.location.href = "Adopter.php?nom=" + nomChien + "&date=" + date + "&heure=" + heure + "&lieu=" + lieu;
    }
}
  </script>
</body>
</html>