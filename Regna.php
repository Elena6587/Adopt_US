<!DOCTYPE html>
<html lang="fr-FR">
<head>
  <meta charset="UTF-8">
  <title>Adopt US</title>
  <link rel="stylesheet" href="css/Regna.css">
</head>
<body>
  <h1>Chien</h1>
    <?php require_once 'header.php';?>

  <div class="card">
    <div class="card-image">
        <img src="Chihuahua noir marron ou orange.jpg" alt="Chihuahua noir marron ou orange">
    </div>
    <div class="card-content">
        <h3>Race: Chihuahua</h3>
        <h4>NOM: Regna</h4>
        <p>AGE: 11 ans</p>
        <p>SEXE: FEMELLE</p>
        <p>TAILLE: 18 cm</p>
        <p>POIDS: 1.05 kg</p>
        <button href="Adopter.php" onclick="demanderAdoption('Regna')" class="button-slide">Adopter</a>
    </div>
</div>
<script>
function demanderAdoption(nomChien) {
    let confirmation = confirm("Confirmer l'adoption de " + nomChien + " ?");
    if (confirmation) {
        // On définit les infos du rendez-vous
        const date = "22 Octobre 2026";
        const heure = "13h15";
        const lieu = "123 Rue de la Paix, Paris";
        // On envoie tout vers la page Adopter.php dans l'URL
        window.location.href = "Adopter.php?nom=" + nomChien + "&date=" + date + "&heure=" + heure + "&lieu=" + lieu;
    }
}
  </script>
</body>
</html>